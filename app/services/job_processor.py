...
class JobProcessor:
    def __init__(
        self, repo: SQLiteRepository, hh: HHClient, bot: Bot, /, per_page: int = 100
    ) -> None:
        self._repo = repo
        self._hh = hh
        self._bot = bot
            ...
            for v in vacancies:
                ...
                try:
                    ...
                    await self._hh.apply(
                        token.access_token, vacancy_id, filters.get("resume_id")
                    )
                    ...
                    await self._bot.send_message(
                        ...
                        f"Не удалось откликнуться на вакансию {vacancy_id}: {e}",
                    )
                    continue
                else:
